...
public class KController {
	...
	static Logger logger = LoggerFactory.getLogger(KController.class);
	...
	public String loginException(){
		return "k-loginException";
	}
	
	
	@RequestMapping(value="/k-index", method=RequestMethod.GET)
	...
	public String detailBoard(Model model, int boardNo) {
		...
		return "k-writeReboard";
	}

	@RequestMapping(value = "/writeReboard", method = RequestMethod.POST)
	...
	@RequestMapping(value ="/lanuageBoard", method=RequestMethod.GET)
	...
	@RequestMapping(value ="/detailMyBoard", method=RequestMethod.GET)
	public String detailMyBoard(Model model, HttpSession session,int boardNo){
		logger.trace("detailMyBoard method");
		Board board = bDao.selectBoard(boardNo);
		model.addAttribute("board", board);
		model.addAttribute("reboards",rDao.selectReboard(boardNo));
		return "k-detailMyBoard";
	}
	
	@RequestMapping(value ="/myDetailBoardReturn", method=RequestMethod.GET)
	...
	public void mailSender(HttpServletRequest request, ModelMap mo,
		...
		String subject = "코멘트를 수정 받았습니다."; 					  //메일 제목 입력해주세요.
		String body = "웹사이트에 접속해 확인하세요"; //메일 내용 입력해주세요.
